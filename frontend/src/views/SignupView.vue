<template>
  <div class="signin">
    ...
    <div class="card">
      <div class="card-body">
        <h2 class="formField__msg" v-if="mode == 'signup'">
          Vous avez déjà un compte ?<span class="text-danger text-decoration-underline" @click="switchToLogin">
            Connexion par ici</span
          >
        </h2>
        <h2 class="formField__msg" v-else>
          Vous n'avez pas encore de compte ?<span class="text-danger text-decoration-underline" @click="switchToSignup">
        ...
        <form action="">
          ...
          <button type="submit" class="btn btn-primary fs-4" v-if="mode == 'signup'" @click.prevent="sendSigninform">
            inscription
          </button>
  ...
  methods: {
    ...
    validPassword: function (e) {
      ...
      if (password) {
        ...
        let result =
          containsUppercase && containsLowercase && containsNumber && containsSpecialCaracter && contains8caracters;
        if (result == true) {
          console.log("password : valide");
          this.passwordErrorMsg = "";
        } else if (result == false) {
          this.passwordErrorMsg =
            "Le mot de passe est invalide. Ecrivez plus que 8 caractaires avec des caractères, des nombres et des caractères speciales suivantes -/*._ et @ .";
        } else {
    ...
    sendSigninform: function (e) {
      ...
      this.$store.dispatch("sendSigninform", {
        //$store.dispatch method triggers actions
        name: this.name,
        email: this.email,
